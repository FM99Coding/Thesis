...
def fetch_data_from_onos():
    ...
    print("Fetching initial data from ONOS...")
    ...
    try:
        ...
        print(f"Active devices found: {active_devices}")
        ...
        print("Initial flow rules data fetched successfully!")
        ...
        try:
            ...
            with flows_lock: #updates flow dataframe
                ...
                if flows_to_remove: #if a flow currently registered is not seen by ONOS, it should be removed
                    ...
                    print(f"Removed flows: {list(flows_to_remove)}")
                ...
                for flow in onos_flows_data:
                    ...
                    if flow_id not in current_flow_ids: #register new flows
                        ...
                        print(f"Added new flow rule: {flow_id} on device {device_id}")
                        ...
                        print(f"Updated stats for flow rule: {flow_id}")

                if new_flows_list:
                    df_flows=pd.concat([df_flows, pd.DataFrame(new_flows_list)], ignore_index=True) #adds rules to dataframe

            print("Flow rules data updated successfully.")

        except requests.exceptions.RequestException as e:
            print(f"Error updating flow rules list: {e}")
