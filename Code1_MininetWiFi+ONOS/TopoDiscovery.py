...
df_stations=pd.DataFrame(columns=['ID', 'MAC', 'IPv4', 'Connection_Point',
                                  'Name', 'Type', 'Interface',
                                  'Position', 'WiFi_Mode', 'WiFi_Frequency[GHz]', 'RSSI[dBm]',
    ...
    for host in onos_hosts_data:
        ...
        if type=="Host":
            ...
            if "position" in host.get("annotations", {}):
                ...
                pos, mode, freq, rssi, tx_bytes, rx_bytes, tx_packets, rx_packets,\
...
def read_ports(onos_ports_data):
    ...
    for port in onos_ports_data: #for every L2 port
        ...
        if device.empty: #if the OVS device of the current port cannot be found in the memorized data
            print(f"{dt.now().strftime('%H:%M:%S')} -> Error: unable to find device {ovs_device} for port {port_name}")
...
def read_links(onos_links_data):
    ...
    for link in onos_links_data: #for every Ethernet link
        ...
        if src_port_data.empty or dst_port_data.empty: #if the source port and/or the dst port cannot be found in the dataframe
            print(f"{dt.now().strftime('%H:%M:%S')} -> Error: unable to find ports for link {src_device}/{src_port_num}-{dst_device}/{dst_port_num}")
...
def read_connections():
    ...
    for index, row in df_wlan_ports.iterrows(): #for every WiFi port
        ...
        if not connected_devices_list: #if no station device is connected to this WiFi port
            print(f"{dt.now().strftime('%H:%M:%S')} -> Error: unable to find connected hosts for Connection Point: {connection_point}")
...
def update_topology():
    ...
    while True:
        ...
        try:
            ...
            print(f"{dt.now().strftime('%H:%M:%S')} -> Topology summary updated successfully.")

        except requests.exceptions.RequestException as e:
            print(f"{dt.now().strftime('%H:%M:%S')} -> Error updating topology summary: {e}")
        ...
        try:
            ...
            with switches_lock:
                ...
                if switches_to_remove: #if there are OVS devices to be removed
                    ...
                    print(f"{dt.now().strftime('%H:%M:%S')} -> Removed switches: {switches_to_remove}")
                ...
                for sw in onos_switches_data: #for all OVS devices seen by ONOS
                    ...
                    if dpid not in current_dpid_set: #if the OVS sw is new (not yet registered in the dataframe)
                        ...
                        print(f"{dt.now().strftime('%H:%M:%S')} -> Added new switch: {dpid}")

                if new_switches_list:
                    df_switches=pd.concat([df_switches, pd.DataFrame(new_switches_list)], ignore_index=True) #adds new rows

            print(f"{dt.now().strftime('%H:%M:%S')} -> Switch data successfully updated")

        except requests.exceptions.RequestException as e:
            print(f"{dt.now().strftime('%H:%M:%S')} -> Error updating switches list: {e}")
        ...
        try:
            ...
            with aps_lock:
                ...
                if aps_to_remove: #if there are OVS devices to be removed
                    ...
                    print(f"{dt.now().strftime('%H:%M:%S')} -> Removed aps: {aps_to_remove}")
                ...
                for ap in onos_aps_data: #for all OVS devices seen by ONOS
                    ...
                    if dpid not in current_dpid_set: #if the OVS ap is new (not yet registered in the dataframe)
                        ...
                        print(f"{dt.now().strftime('%H:%M:%S')} -> Added new AP: {dpid}")

                if new_aps_list:
                    df_aps=pd.concat([df_aps, pd.DataFrame(new_aps_list)], ignore_index=True) #adds new rows

            print(f"{dt.now().strftime('%H:%M:%S')} -> APS data successfully updated")

        except requests.exceptions.RequestException as e:
            print(f"{dt.now().strftime('%H:%M:%S')} -> Error updating APs list: {e}")
        ...
        try:
            ...
            with hosts_lock:
                ...
                if hosts_to_remove: #if there are host devices to be removed
                    ...
                    print(f"{dt.now().strftime('%H:%M:%S')} -> Removed host devices: {hosts_to_remove}")
                ...
                for host in host_to_process: #for all host devices seen by ONOS
                    ...
                    if id not in current_id_set: #if the host device is new (not yet registered in the dataframe)
                        ...
                        print(f"{dt.now().strftime('%H:%M:%S')} -> Added new host device: {id}")
            ...
            print(f"{dt.now().strftime('%H:%M:%S')} -> Hosts data successfully updated")

        except requests.exceptions.RequestException as e:
            print(f"{dt.now().strftime('%H:%M:%S')} -> Error updating hosts list: {e}")
        ...
        try:
            ...
            with stas_lock:
                ...
                if stations_to_remove: #if there are station devices to be removed
                    ...
                    print(f"{dt.now().strftime('%H:%M:%S')} -> Removed station devices: {stations_to_remove}")
                ...
                for station in onos_stations_data: #for all station devices seen by ONOS
                    ...
                    if id not in current_id_set: #if the station device is new (not yet registered in the dataframe)
                        if "annotations" in station:
                            ...
                            intf, pos, mode, freq, rssi, tx_bytes, rx_bytes, tx_packets, rx_packets, \
                        ...
                        print(f"{dt.now().strftime('%H:%M:%S')} -> Added new station device: {id}")
            ...
            print(f"{dt.now().strftime('%H:%M:%S')} -> Stations data updated successfully")

        except requests.exceptions.RequestException as e:
            print(f"{dt.now().strftime('%H:%M:%S')} -> Error updating stations list: {e}")
        ...
        try:
            ...
            with eth_ports_lock:
                ...
                if ports_to_remove: #if there are Ethernet ports to be removed
                    ...
                    print(f"{dt.now().strftime('%H:%M:%S')} -> Removed ports: {ports_to_remove}")
                ...
                for port_name, port in onos_ports_map.items(): #for all Ethernet ports seen by ONOS
                    ...
                    if not device_exists or not port.get("isEnabled"): #if OVS device does not exist, or the port is not configured
                        if port_name in current_ports_map: #if current Ethernet port is already registered
                            ...
                            print(f"{dt.now().strftime('%H:%M:%S')} -> Removed inactive/invalid Ethernet port: {port_name}")
                    ...
                    if port_name not in current_ports_map: #if the Ethernet port is new, it has to be registered in the DataFrame
                        ...
                        print(f"{dt.now().strftime('%H:%M:%S')} -> Added new port: {port_name}")
            ...
            print(f"{dt.now().strftime('%H:%M:%S')} -> Ethernet ports data updated successfully")
        ...
        try:
            ...
            with eth_ports_lock:
                ...
                if ports_to_remove: #if there are WiFi ports to be removed
                    ...
                    print(f"{dt.now().strftime('%H:%M:%S')} -> Removed WiFi ports: {ports_to_remove}")
                ...
                for port_name, port in onos_ports_map.items(): #for all WiFi ports seen by ONOS
                    ...
                    if not device_exists or not port.get("isEnabled"): #if OVS device does not exist, or the port is not configured
                        if port_name in current_ports_map: #if current WiFi port is already registered
                            ...
                            print(f"{dt.now().strftime('%H:%M:%S')} -> Removed inactive/invalid WiFi port: {port_name}")
            ...
            print(f"{dt.now().strftime('%H:%M:%S')} -> WiFi ports data updated successfully")

        except requests.exceptions.RequestException as e:
            print(f"{dt.now().strftime('%H:%M:%S')} -> Error updating ports list: {e}")
        ...
        try:
            ...
            with links_lock:
                ...
                if links_to_remove: #if there are links to be removed
                    ...
                    print(f"{dt.now().strftime('%H:%M:%S')} -> Removed links: {links_to_remove}")
                ...
                for link_name, link in onos_links_dict.items(): #for all links seen by ONOS (bidirectional links are reported just once)
                    ...
                    with eth_ports_lock:
                        ...
                        if src_port_data.empty or dst_port_data.empty: #if source/destination port is not recorded
                            if link_name in current_links_map: #if the current link is already in the DataFrame
                                ...
                                print(f"{dt.now().strftime('%H:%M:%S')} -> Removed link with invalid ports: {link_name}")
                    ...
                    if link_name not in current_links_map: #if the link is new (registered for the first time in the DataFrame)
                        ...
                        print(f"{dt.now().strftime('%H:%M:%S')} -> Added new link: {link_name}")
            ...
            print(f"{dt.now().strftime('%H:%M:%S')} -> Ethernet links data updated successfully")

        except requests.exceptions.RequestException as e:
            print(f"{dt.now().strftime('%H:%M:%S')} -> Error updating links list: {e}")
...
def update_connections():
    ...
    while True:
        ...
        try:
            ...
            with connections_lock:
                ...
                with wlan_ports_lock:
                    ...
                    if connections_to_remove: #if there are WiFi connections to be removed
                        ...
                        print(f"{dt.now().strftime('%H:%M:%S')} -> Removed inactive connections: {connections_to_remove}")
                    ...
                    for index, row in df_wlan_ports.iterrows(): #for every up-to-date WiFi connection
                        ...
                        if not connected_stas_list: #if no station is exploiting this connection
                            if interface_name in current_connections_map: #if current connection is already registered in the DataFrame
                                ...
                                print(f"{dt.now().strftime('%H:%M:%S')} -> Removed connection with no hosts: {interface_name}")
                        ...
                        if interface_name not in current_connections_map: #if the WiFi connection is new
                            ...
                            print(f"{dt.now().strftime('%H:%M:%S')} -> Added new WiFi connection: {interface_name}")
            ...
            print(f"{dt.now().strftime('%H:%M:%S')} -> WiFi Connections data updated successfully")

        except requests.exceptions.RequestException as e:
            print(f"{dt.now().strftime('%H:%M:%S')} -> Error updating connections list: {e}")
