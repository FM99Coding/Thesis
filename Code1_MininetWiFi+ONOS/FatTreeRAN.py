...
###Logging/scripting metadata
cur_dir=os.path.dirname(os.path.abspath(__file__)) #current directory
log_dir=os.path.join(cur_dir, 'logs') #directory of logs data
script_dir=os.path.join(os.path.dirname(cur_dir), 'Thesis2') #directory containing external scripts to be launched
venv_python=os.path.join(script_dir, '.venv_py39', 'bin', 'python') #python binary to be used to run the scripts
...
def topology(args):
    ...
    if '--plot' in args:
        net.plotGraph(min_x=-(x1+dx*(num_aps+1))*0.1, min_y=0,
                      max_x=(x1+dx*(num_aps+1))*1.1, max_y=y_ap*3) #visual representation of RAN segment in a 2D space
...
'''Start northbound applications that interact with the network through the ONOS controller:
   -> TopoDiscovery.py (expose real-time info about emulated network)
   -> FlowDiscovery.py (expose real-time info about installed OpenFlow rules)
   -> ECMPRouting.py (apply Equal-Cost Multi-Path routing on the network, computing paths and installing OF rules
   @return subprocess[] list of processes'''
def start_external_scripts():
    global script_dir, log_dir, venv_python

    os.makedirs(log_dir, exist_ok=True) #create log directory if it does not exist

    ###Write log files
    topo_log=open(os.path.join(log_dir, 'TopoDiscovery.log'), 'w')
    flow_log=open(os.path.join(log_dir, 'FlowDiscovery.log'), 'w')
    ecmp_log=open(os.path.join(log_dir, 'ECMProuting.log'), 'w')

    processes=[] #list of subprocess objects

    ###Start northbound scripts and redirects stdout and stderr to log files
    processes.append(subprocess.Popen([venv_python, os.path.join(script_dir, 'TopoDiscovery.py')],
                                      stdout=topo_log, stderr=subprocess.STDOUT))
    processes.append(subprocess.Popen([venv_python, os.path.join(script_dir, 'FlowDiscovery.py')],
                                      stdout=flow_log, stderr=subprocess.STDOUT))
    processes.append(subprocess.Popen([venv_python, os.path.join(script_dir, 'ECMProuting.py')],
                                      stdout=ecmp_log, stderr=subprocess.STDOUT))

    print("Activation of north-bound processes completed\n")
    return processes
...
def update_aps():
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            print(f'Handled error {a}\n')
            pass #continue thread execution
...
if __name__ == '__main__':
    ...
    print(f'Initialize north-bound applications\n')
    time.sleep(30)
    proc_list=start_external_scripts()

    CLI(net) #opens Mininet CLI

    ###Closing threads upon exiting Mininet CLI
    stop_event.set() #signal the threads to stop
    print(f'Stopping threads: {stop_event}\n')
    for t in threads:
        t.join() #wait for all threads to finish

    ###Terminating northbound processes
    print("Terminating northbound processes...\n")
    for p in proc_list:
        p.terminate() #send termination signal

    for p in proc_list:
        p.wait() #wait process termination
