...
df_devices=pd.DataFrame(columns=['DPID', 'Name', 'Type', 'Model', 'Manufacturer', 'Control Protocol']) #dataframe of OVS devices
df_hosts=pd.DataFrame(columns=['ID', 'MAC Address', 'IPv4 Addresses', 'Connection Point']) #dataframe of host devices
df_ports=pd.DataFrame(columns=['Name', 'OVS Device', 'Type', 'Port Number', 'MAC', 'Port Speed [Mbps]',
                               'TX_Bytes', 'RX_Bytes', 'Sampling Time [s]']) #dataframe of L2 ports
df_links=pd.DataFrame(columns=['Name', 'Source Device', 'Source Port', 'Destination Device', 'Destination Port', 'Type',
                               'Bandwidth [Mbps]', 'Raw Data [Byte]', 'Throughput [Mbps]', 'Utilization [%]']) #dataframe of Eth links
df_connections=pd.DataFrame(columns=['Interface', 'Access Point', 'Access Port', 'Connected Devices', 'Type', 'Bandwidth [Mbps]',
                                     'Raw Data [Byte]', 'Throughput [Mbps]', 'Utilization [%]']) #dataframe of WiFi connections
...
def fetch_data_from_onos():
    ...
    global df_ports
    global df_links
    global df_connections

    print("Fetching data from ONOS at startup...")

    try:
        ############################################################################################Get topology summary
        summary_response=requests.get(onos_system_url, auth=onos_auth, headers=headers) #fetches data from ONOS endpoint
        ...
        for device in onos_devices_data:
            ...
            devices_list.append({
                ...
                "Control Protocol": device.get("annotations", {}).get("protocol") })

        if devices_list:
            df_devices=pd.DataFrame(devices_list) #populates OVS device dataframe with fetched data
        ...
        for host in onos_hosts_data:
            hosts_list.append({
                ...
                "IPv4 Addresses": host.get("ipAddresses", []),
                "Connection Point": f"{host.get('locations', {})[0].get('elementId', '')}/"
        ...
        ports_response=requests.get(onos_ports_url, auth=onos_auth, headers=headers) #fetches data from ONOS endpoint
        ...
        for port in onos_ports_data: #for every L2 port
            ...
            device=df_devices[df_devices['DPID']==ovs_device] #extract the target device from the OVS devices dataframe
            ...
            stats_response=requests.get(stats_url, auth=onos_auth, headers=headers) #fetches data from ONOS endpoint
            ports_response.raise_for_status() #raises HTTP exception
            ...
            ports_list.append({
                ...
                "Sampling Time [s]": port_stats_data.get("durationSec") })

        if ports_list:
            df_ports=pd.DataFrame(ports_list) #populates ports dataframe with fetched data
        ...
        for link in onos_links_data: #for every Ethernet link
            ...
            if (dst_device, dst_port_num, src_device, src_port_num) in links_set: #if the reversed 4-tuple is already in the set (same link, opposite direction)
            ...
            src_port_data=df_ports[(df_ports['OVS Device']==src_device) & (df_ports['Port Number']==src_port_num)]
            dst_port_data=df_ports[(df_ports['OVS Device']==dst_device) & (df_ports['Port Number']==dst_port_num)]
            ...
            if src_port_data.empty or dst_port_data.empty: #if the source port and/or the dst port cannot be found in the dataframe
                ...
                sampling_interval=src_port_data['Sampling Time [s]'].iloc[0]
            ...
            links_list.append({
                ...
                "Source Device": src_device,
                "Source Port": src_port_num,
                "Destination Device": dst_device,
                "Destination Port": dst_port_num,
                "Type": "Ethernet",
                "Bandwidth [Mbps]": bandwidth,
                "Raw Data [Byte]": tx_bytes+rx_bytes,
        ...
        wifi_ports=df_ports[df_ports['Type']=='WiFi'] #filters only WiFi ports on OVS access points

        for index, row in wifi_ports.iterrows(): #for every WiFi port
            access_point=row['OVS Device']
            access_port=row['Port Number']
            connection_point=f"{access_point}/{access_port}"

            connected_hosts=df_hosts[df_hosts['Connection Point']==connection_point] #host devices connected to this WiFi port
            ...
            sampling_interval=row['Sampling Time [s]'] #s
            ...
            wifi_connection_data = {
                ...
                'Connected Devices': connected_devices_list,
                'Type': 'WiFi',
                'Bandwidth [Mbps]': bandwidth,
                'Raw Data [Byte]': tx_bytes+rx_bytes,
                'Throughput [Mbps]': throughput,
...
def update_topology():
    ...
    while True:
        ...
        try:
            ...
            summary_response=requests.get(onos_system_url, auth=onos_auth, headers=headers)
...
'''Periodically update df_devices DataFrame (content of endpoint http://localhost:8080/topology/devices)'''
def update_devices():
    global df_devices
    ...
    while True:
        ...
        try:
            ...
            new_devices_list=[] #list of newly discovered OVS devices
            with devices_lock:
                current_dpid_set=set(df_devices['DPID']) #DPIDs of currently registered OVS devices
                onos_dpid_set={device.get("id") for device in onos_devices_data} #OVS devices that are still seen by ONOS
                devices_to_remove=list(current_dpid_set-onos_dpid_set) #if ONOS does not see the device anymore, it has to be removed

                if devices_to_remove: #if there are OVS devices to be removed
                    df_devices.set_index('DPID', inplace=True) #sets column DPID as DataFrame index (more efficient removal)
                    df_devices.drop(devices_to_remove, inplace=True) #remove DataFrame rows corresponding to stale OVS devices
                    df_devices.reset_index(inplace=True) #reset original DataFrame index
                    print(f"Removed devices: {devices_to_remove}")

                for device in onos_devices_data: #for all OVS devices seen by ONOS
                    dpid=device.get("id")

                    if dpid not in current_dpid_set: #if the OVS device is new (not yet registered in the dataframe)
                        ...
                        new_row={
                            ...
                            "Model": f"{device.get('hw', '')} {device.get('sw', '')}".strip(),
                            "Manufacturer": device.get("mfr"),
                            "Control Protocol": device.get("annotations", {}).get("protocol")} #create new row for the OVS device

                        new_devices_list.append(new_row)
                        print(f"Added new device: {dpid}")

                if new_devices_list:
                        df_devices=pd.concat([df_devices, pd.DataFrame(new_devices_list)], ignore_index=True) #adds new rows
                ...
                for host in onos_hosts_data: #for all host devices seen by ONOS
                    ...
                    if id not in current_id_set: #if the host device is new (not yet registered in the dataframe)
                        new_row={
                            ...
                            "IPv4 Addresses": host.get("ipAddresses", []),
                            "Connection Point": f"{host.get('locations', {})[0].get('elementId', '')}/"
                                                f"{host.get('locations', {})[0].get('port')}".strip() } #create row for new host device
                        ...
                        if not df_hosts.loc[host_row, 'Connection Point'].iloc[0]==connection_point: #if the host is not anymore connected to the same OVS device
                            df_hosts.loc[host_row, 'Connection Point']=connection_point #updates host's connection point
...
'''Periodically update df_ports DataFrame (content of endpoint http://localhost:8080/topology/ports)'''
def update_ports():
    global df_ports, df_devices
    ...
    while True:
        ...
        try:
            ...
            ports_response=requests.get(onos_ports_url, auth=onos_auth, headers=headers)
            ports_response.raise_for_status()
            onos_ports_data=ports_response.json().get("ports", [])

            new_ports_list=[] #list of newly discovered L2 ports on OVS devices
            with ports_lock:
                current_ports_map={row['Name']: row.name for index, row in df_ports.iterrows()} #<port name, row index> for all ports in the DataFrame
                ...
                if ports_to_remove: #if there are L2 ports to be removed
                    df_ports.set_index('Name', inplace=True) #sets column Name as DataFrame index (more efficient removal)
                    df_ports.drop(list(ports_to_remove), inplace=True) #remove DataFrame rows corresponding to stale L2 ports
                    df_ports.reset_index(inplace=True) #reset original DataFrame index
                ...
                for port_name, port in onos_ports_map.items(): #for all L2 ports seen by ONOS
                    ...
                    with devices_lock:
                        device_exists=not df_devices[df_devices['DPID']==ovs_device].empty #get OVS devices were current L2 port is placed
                    ...
                    if not device_exists or not port.get("isEnabled"): #if the corresponding OVS device does not exist, or if the port is not configured for traffic
                        if port_name in current_ports_map: #if current port is already registered, it has to be removed from the DataFrame
                            df_ports.drop(index=current_ports_map[port_name], inplace=True) #remove DataFrame rows corresponding to inactive ports
                    ...
                    stats_url=f"{onos_port_stats_url}/{urllib.parse.quote(ovs_device)}/{port_num}"
                    stats_response=requests.get(stats_url, auth=onos_auth, headers=headers)
                    ...
                    if port_name not in current_ports_map: #if the L2 port is new, it has to be registered in the DataFrame
                        ...
                        new_row={
                            ...
                            "TX_Bytes": port_stats_data.get("bytesSent"),
                            "RX_Bytes": port_stats_data.get("bytesReceived"),
                            "Sampling Time [s]": port_stats_data.get("durationSec")} #row for the new port

                        new_ports_list.append(new_row)
                        print(f"Added new port: {port_name}")

                    else: #if the port is already registered and still active
                        df_ports.loc[current_ports_map[port_name], 'TX_Bytes']=port_stats_data.get("bytesSent") #updates counter of tx bytes
                        df_ports.loc[current_ports_map[port_name], 'RX_Bytes']=port_stats_data.get("bytesReceived") #updates counter of rx bytes
                        df_ports.loc[current_ports_map[port_name], 'Sampling Time [s]']=port_stats_data.get("durationSec") #updates sampling time record
                        print(f"Updated stats for port: {port_name}")

                if new_ports_list:
                    df_ports=pd.concat([df_ports, pd.DataFrame(new_ports_list)], ignore_index=True) #adds rows to the DataFrame
        ...
        try:
            ...
            with links_lock:
                ...
                for link_name, link in onos_links_dict.items(): #for all links seen by ONOS (bidirectional links are reported just once)
                    ...
                    with ports_lock:
                        src_port_data=df_ports[(df_ports['OVS Device']==src_device) & (df_ports['Port Number']==src_port_num)] #gets source port info
                        dst_port_data=df_ports[(df_ports['OVS Device']==dst_device) & (df_ports['Port Number']==dst_port_num)] #gets destination port info
                    ...
                    if link_name not in current_links_map: #if the link is new (registered for the first time in the DataFrame)
                        ...
                        previous_data=df_links.loc[current_links_map[link_name], 'Raw Data [Byte]']
                        throughput=((tx_bytes+rx_bytes-previous_data)*(8/30))/1000000 #[Mbps]
                        utilization=(throughput/bandwidth)*100 #[%]

                        df_links.loc[current_links_map[link_name], 'Raw Data [Byte]']=tx_bytes+rx_bytes #updates field in the DataFrame
...
def update_connections():
    global df_connections, df_ports, df_hosts
    ...
    while True:
        ...
        try:
            ...
            with connections_lock:
                ...
                with ports_lock:
                    wifi_ports=df_ports[df_ports['Type']=='WiFi'] #filters only WiFi L2 ports from df_ports DataFrame
                ...
                for index, row in wifi_ports.iterrows(): #for every up-to-date WiFi connection
                    ...
                    with hosts_lock:
                        connected_hosts=df_hosts[df_hosts['Connection Point']==connection_point] #gets all up-to-date host devices on current WiFi connection
                    ...
                    tx_bytes=row['TX_Bytes']
                    rx_bytes=row['RX_Bytes']
                    sampling_interval=row['Sampling Time [s]']
                    ...
                    if interface_name not in current_connections_map: #if the WiFi connection is new
                        ...
                        new_row={
                            ...
                            "Access Point": access_point,
                            "Access Port": access_port,
                            "Connected Devices": connected_hosts_list,
                            "Type": "WiFi",
                            "Bandwidth [Mbps]": bandwidth,
                            "Raw Data [Byte]": tx_bytes+rx_bytes,
                            "Throughput [Mbps]": throughput,
                        ...
                        previous_data=df_connections.loc[current_connections_map[interface_name], 'Raw Data [Byte]'] #Byte
                        throughput=((tx_bytes+rx_bytes-previous_data)*(8/30))/1000000 #[Mbps]
                        utilization=(throughput/bandwidth)*100 #[%]

                        df_connections.at[current_connections_map[interface_name], 'Connected Devices']=connected_hosts_list #updates field
                        df_connections.loc[current_connections_map[interface_name], 'Raw Data [Byte]']=tx_bytes+rx_bytes #updates field
...
def get_ovs_devices():
    return jsonify(df_devices.to_dict(orient="records")) #converts dataframe to JSON to be exposed
...
def get_l2_ports():
    return jsonify(df_ports.to_dict(orient="records"))
...
def get_ports_by_device(deviceId):
    with ports_lock:
        device_ports=df_ports[df_ports['OVS Device']==deviceId] #filters only l2 ports of the specified device
...
if __name__=="__main__":
    ...
    up_devices=threading.Thread(target=update_devices, daemon=True) #update OVS devices dataframe
