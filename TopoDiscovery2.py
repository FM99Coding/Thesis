...
df_switches=pd.DataFrame(columns=['DPID', 'Name', 'Type', 'Model', 'Manufacturer', 'Control Protocol']) #dataframe of OVS switches
df_aps=pd.DataFrame(columns=['DPID', 'Name', 'Type', 'Model', 'Manufacturer', 'Control Protocol', 'Position']) #dataframe of OVS APs

df_hosts=pd.DataFrame(columns=['ID', 'MAC Address', 'IPv4 Address', 'Connection Point',
                               'Name', 'Type', 'L2 Interface']) #dataframe of host devices
df_stations=pd.DataFrame(columns=['ID', 'MAC Address', 'IPv4 Address', 'Connection Point',
                                  'Name', 'Type', 'L2 Interface',
                                  'Position', 'WiFi Mode', 'WiFi Frequency', 'RSSI', 'AP SSID', 'AP Distance'
                                  'Tx Bytes', 'Rx Bytes', 'Tx Packets', 'Rx Packets',
                                  'Tx_Errors', 'Rx Errors', 'Tx_Drops', 'Rx_Drops', 'Collisions']) #dataframe of station devices

df_eth_ports=pd.DataFrame(columns=['Name', 'OVS Device', 'Type', 'Port Number', 'MAC', 'TX_Bytes', 'RX_Bytes',
                                   'Tx Packets', 'Rx Packets', 'Tx_Errors', 'Rx Errors', 'Tx_Drops', 'Rx_Drops', 'Sampling Time [s]',
                                   'Max Throughput', 'Max Queue Length', 'Backlog Packets', 'Backlog Bytes']) #dataframe of L2 Ethernet ports

df_wlan_ports=pd.DataFrame(columns=['Name', 'OVS Device', 'Type', 'Port Number', 'MAC', 'TX_Bytes', 'RX_Bytes',
                                    'Tx Packets', 'Rx Packets', 'Tx_Errors', 'Rx Errors', 'Tx_Drops', 'Rx_Drops', 'Sampling Time [s]',
                                    'Max Throughput', 'Max Queue Length', 'Backlog Packets', 'Backlog Bytes',
                                    'WiFi Mode', 'WiFi Frequency', 'WiFi Channel', 'Bandwidth', 'Tx Power', 'Gain',
                                    'SSID', 'Range' 'Collisions']) #dataframe of L2 wireless ports
    ...
    try:
        ...
        for port in onos_ports_data: #for every L2 port
            ...
            if port_type=="Ethernet":
                device=df_switches[df_switches['DPID']==ovs_device] #extract the target device from the OVS devices dataframe
            else:
        ...
        for link in onos_links_data: #for every Ethernet link
            ...
            src_port_data=df_eth_ports[(df_eth_ports['OVS Device']==src_device) & (df_eth_ports['Port Number']==src_port_num)]
            dst_port_data=df_eth_ports[(df_eth_ports['OVS Device']==dst_device) & (df_eth_ports['Port Number']==dst_port_num)]
    ...
    for host in onos_hosts_data:
        ...
        if type=="Host":
            hosts_list.append({
                ...
                "Name": name,
                "Type": type,
                "IPv4 Addresses": host.get("ipAddresses", [None])[0],
            ...
            if "position" in host.get("annotations", {}):
                ...
                rssi=host.get("annotations", {}).get("RSSI")
        ...
        try:
            ...
            with switches_lock:
                ...
                if devices_to_remove: #if there are OVS devices to be removed
                    ...
                    df_switches.drop(devices_to_remove, inplace=True) #remove DataFrame rows corresponding to stale OVS devices
                ...
                if new_devices_list:
                        df_switches=pd.concat([df_switches, pd.DataFrame(new_devices_list)], ignore_index=True) #adds new rows
            ...
            with eth_ports_lock:
                ...
                for port_name, port in onos_ports_map.items(): #for all L2 ports seen by ONOS
                    ...
                    if port_name not in current_ports_map: #if the L2 port is new, it has to be registered in the DataFrame
                        ...
                        df_eth_ports.loc[current_ports_map[port_name], 'Sampling Time [s]']=port_stats_data.get("durationSec") #updates sampling time record
        ...
        try:
            ...
            with links_lock:
                ...
                for link_name, link in onos_links_dict.items(): #for all links seen by ONOS (bidirectional links are reported just once)
                    ...
                    with eth_ports_lock:
                        src_port_data=df_eth_ports[(df_eth_ports['OVS Device']==src_device) & (df_eth_ports['Port Number']==src_port_num)] #gets source port info
                        dst_port_data=df_eth_ports[(df_eth_ports['OVS Device']==dst_device) & (df_eth_ports['Port Number']==dst_port_num)] #gets destination port info
...
def get_ports_by_device(deviceId):
    with eth_ports_lock:
        device_ports=df_eth_ports[df_eth_ports['OVS Device']==deviceId] #filters only l2 ports of the specified device
